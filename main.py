...
def parse_cin(path='boshiamy.cin'):
    result = {}
    in_chardef = False
...
def save_settings(mode_idx, random_mode, letter_start, letter_end):
    try:
        with open(SETTINGS_FILE, 'w', encoding='utf-8') as f:
            json.dump({'mode': mode_idx, 'random': random_mode,
                       'letter_start': letter_start, 'letter_end': letter_end}, f)
    except Exception:
        pass
...
def main():
    ...
    layout = [
        [sg.Text('無蝦米快打練習', font=('Helvetica', 16)),
         sg.Button('離開', key='-EXIT-', font=('Helvetica', 12))],
    ...
    def check_answer():
        ...
        if typing == keystroke:
            ...
            if mistakes >= 3:
                ...
                typing = ''
                show_answer_until = time.time() + 1.5
            else:
    ...
    def do_save():
        save_settings(last_mode_idx, last_random, last_ls, last_le)
